  ...
  return new Promise((resolve, reject) => {
    lock(fname).then((release) => {
      fs.writeFile(fname, JSON.stringify(graphics, null, 2), (err) => {
        if (err) {
          error(fname + ": error saving graphics: " + err.message);
          reject(err.message);
        } else {
          resolve();
        }
...
export function graphicsDelete(fname: string): Promise<any> {
  return new Promise((resolve, reject) => {
    fs.writeFile(fname, "{}", err => {
      if (err) {
        error(fname + ": error deleting graphics: " + err.message);
        reject(err.message);
      } else {
        resolve();
      }
...
export function graphicsElementDelete(fname: string, elId: Id): Promise<any> {
  return fileOpWithLock(fname, new Promise((resolve, reject) => {
    ...
    delete model[elId];
    fs.writeFile(fname, model, err => {
      if (err) {
        error(fname + ": error deleting element graphics: " + err.message);
        reject(err.message);
      } else {
        resolve();
      }
...
export function writeModel(model: any, fname: string): Promise<any> {
  return new Promise((resolve, reject) => {
